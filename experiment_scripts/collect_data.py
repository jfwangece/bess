...
def do_test(i,pps, pcycle):
    ...
    os.system(BESSCTL_DIR + " 'run samples/hw_ts_test BESS_PKT_SIZE=200,BESS_PKT_CYCLE="+str(pcycle)+",BESS_PKT_RATE="+str(pps)+"'")
    sleep(reset_time)
    ...
    os.system(BESSCTL_DIR + " 'command module measure0 get_summary MeasureCommandGetSummaryArg {\"latency_percentiles\":[50,99]}' > output")
